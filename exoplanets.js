window.EXOPLANETS = [
  ...
  },
  {
    ...
    "ly": 43.9,
    "inputs": {"M_star": 1.27, "spin": 18.063182, "f_disc": 0.913447, "stripping": {"M_pert": 0.2, "q": 17.8}},
    "notes": "ENERGY-CLOSING CONFIGURATION (the eviction energy ledger is the discriminator — NOTE: the engine DISPLAYS the corpse-free parsimony twin, B at slot 0 = 8.99 AU; the assignment cannot yet seat a corpse above an outermost stellar body, and energetics are not yet scored. The TRUE configuration sits one rung up, R_disc 15.39, B at slot 1 = the same 8.99 AU seat with the same 0.04% closure and a CRITICAL FITTING ELEMENT for evicted systems — engine queue: eviction-energy closure in scoring): slot 0 (15.39) held a gas-truncated BD-grade primary (core-priced ~430+ ME) that was destabilized and HURLED INTO THE STAR — its star-grazer release pays the eviction bill that the corpse-free fit cannot (B's rise to a~750, e~0.98, q~18 costs ~3,700 GM; survivors' settling + b's delivery supply only ~2,000; the sunk corpse closes the books). Slot 1 (8.99): ups And B, SOURCED BY THE CASCADE at 0.04% (predicted 66,617 vs observed 66,589 ME) — the second cascade-sourced stellar companion after Alpha Cen B — flung up Proxima-style, bound, its periapsis pass (q = 17.8 AU, r_t = 8.99) shearing the outer slots and stirring the survivors (30-deg c-d mutual inclination; abrupt edge at e). Slot 2: e in situ (5.245). Slot 3: d (3.06 -> 2.51). Slot 4/5: c's and b's seats (1.79/1.04); c settled to 0.829; b DELIVERED to 0.059 by the same event (companion-driven Kozai delivery), its seat destroyed behind it. f_disc 0.91: a disc nearly as heavy as its star, as sourcing a stellar companion requires. An 8-10 seat system, most seats dead, eaten, or thrown into the star.",
    "planets": [
