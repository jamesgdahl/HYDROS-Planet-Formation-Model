...
function fragment_clear_zone(r, m, M_star, interior_consumed = false) {
...
function cascade_sites(M_star, spin, min_slots, omega, f_disc, clearings) {
...
function assign_planets_to_slots(planets, M_star, spin, f_disc, omega, clearings) {
...
function slot_aware_fit(planets, M_star, spin, f_disc, opts) {
    ...
    if (!is_inverted_budget(M_star)) {
        ...
        let frag = null; // the single innermost qualifying candidate
        for (const p of planets) {
            ...
            if (p.r > a_bin)
                continue; // beyond the close-fission reach ⇒ not a fragment
            ...
            if (mo >= FRAG_GIANT_MIN && mo > gas_threshold_mass(p.r, M_star, f_disc)) {
                if (frag === null || p.r < frag.r)
                    frag = p; // keep the INNERMOST qualifier
            }
        }
        if (frag)
            frag.fragment = true;
    ...
    if (kbo_bodies.length > 0 || inverted) {
        ...
        if (inverted || is_inverted_budget(M_star)) {
            ...
            const MARCH_SPACING = globalThis.__MARCH_SPACING || 14.0; // mutual Hill radii between products (peas-in-a-pod)
            const MARCH_EXTENT = globalThis.__INV_MARCH || 2.5; // march reach in units of R_A (where products stop forming)
            ...
            const march = [];
            {
                ...
                const ROCK_TAKE = globalThis.__ROCK_TAKE || 0.40; // fraction of the remaining rock each product takes
                const MIN_SEED = globalThis.__MIN_SEED || 0.05; // smallest viable rock seed (M⊕)
            ...
            for (let i = 0; i < march.length; i++) {
                ...
                const comp = classify_slot({ filled: !!matched, slot_r: rr, r_used: matched ? matched.r : rr, observed, stripped,
            ...
            for (const p of kbo_bodies) {
                if (Object.values(matchOf).indexOf(p) >= 0)
                    continue;
...
function budgetFit(planets, budget, lambda, parent, primaryMass) {
    ...
    if (parent == null && primaryMass != null && isFinite(primaryMass)) {
        ...
        if (!is_inverted_budget(M_pre)) {
            ...
            if (lambda != null && isFinite(lambda) && lambda > 0) {
                ...
                set_mdot(accretion_rate(M_pre, Z_pre, fr_pre, disc_fraction_centrifugal(lambda), disc_radius_wind(M_pre), M_pre / SOL_M_PRIMORDIAL, 1.0));
            }
            ...
            for (const p of planets) {
                if (p.core || p.kbo)
                    continue;
                const m = p.observed || 0;
                if (m > 0 && m < M_STELLAR_BOUNDARY && (p.r > a_bin || p.r >= r_snow_pre) && p.r > barrier_r)
                    barrier_r = p.r;
            }
            ...
            const R_pos_frag = (R_A_pre > 0) ? fission_position(M_pre, lambda, R_A_pre) : a_bin;
            let frag = null; // the giant nearest the predicted fission position
            if (in_window)
                for (const p of planets) {
                    if (p.core || p.kbo || p.fragment)
                        continue;
                    ...
                    if (mo < FRAG_GIANT_MIN || mo >= M_STELLAR_BOUNDARY || !(p.r > 0))
                        continue;
                    if (barrier_r > 0 && !(p.r < barrier_r))
                        continue; // spared behind a real outer cascade barrier
                    if (frag === null || Math.abs(Math.log(p.r / R_pos_frag)) < Math.abs(Math.log(frag.r / R_pos_frag)))
                        frag = p;
                }
            if (frag)
                frag.fragment = true; // sub-stellar core fragment ⇒ a shared-core element
    ...
    try {
                ...
                if (m_pred >= FRAG_GIANT_MIN && m_pred <= reservoir) {
                    ...
                    for (const p of planets) {
                        if (p.core || p.kbo)
                            continue;
                        const mo = p.observed || 0;
                        if (mo > 0 && mo < M_STELLAR_BOUNDARY && p.r > outer_r)
                            outer_r = p.r;
                    }
                    ...
                    for (const p of planets) {
                        if (p.core || p.kbo || p.fragment)
                            continue;
                        const mo = p.observed || 0;
                        if (mo < FRAG_GIANT_MIN || mo >= M_STELLAR_BOUNDARY || !(p.r > 0) || !(p.r < outer_r))
                            continue;
                        if (Math.abs(Math.log(p.r / pos)) > 0.405)
                            continue; // position match (±~1.5×)
                        if (Math.abs(mo - m_pred) / m_pred > 0.5)
                            continue; // mass match (±50%)
                        p.fragment = true;
                        matched = true;
                        break;
                    }
        ...
        if (usePhysicsDam && R_disc_phys != null && f_disc_derived != null && M_core_earth > 0 && parent == null) {
            ...
            const nebula = Math.max(0, Mtot_e - primaryMass * M_SUN_EARTH); // budget − star
            const fragment_mass = Math.max(0, M_core_earth - primaryMass * M_SUN_EARTH);
            const lift = fission_lift_fraction(M, omega);
            ...
            if (!inverted) {
                set_disc_metals(halo_metals);
                halo_metals_conserved = halo_metals;
            }
            else
                reset_disc_metals();
            ...
            if (!inverted) {
                ...
                eps_PA_conserved = eps_PA;
                eps_SI_conserved = eps_SI; // remembered for the leftover corrector
        ...
        if (f_disc_derived != null && !inverted && !parent
            ...
            const B_field = dynamo_field_strength(M, omega);
            ...
            if (N_rungs > WAVE_DOUBLING_N_MIN && B_field < B_tongue) {
                m = 2;
                if (N_rungs > WAVE_DOUBLING_N_MIN_4X && B_field < B_PARAMETRIC_CRIT_4X)
                    m = 4;
            }
            if (m > 1) {
                set_wave_doubling(m);
                budget_wave_doubling = m;
            }
        }
        let f;
        let fit;
        if (f_disc_derived != null) {
            ...
            if (halo_metals_conserved >= 0) {
                const consumed = fit.slots.reduce((a, s) => (s.filled && !s.exterior && !s.external) ? a + Math.max(0, s.core || 0) : a, 0);
