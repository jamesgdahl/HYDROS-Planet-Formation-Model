...
function core_mean_density(f_rock) {
...
function fission_beta(lambda, f_rock = COMP_F_ROCK) {
...
function in_fission_edge(M_star_msun, lambda, R_A) {
    if (!(R_A > 0))
        return false;
    if (corotation_radius(M_star_msun, lambda) / R_A < FISSION_EDGE_LO)
        return false;
    ...
    const r_roche = roche_limit_au(M_star_msun);
    if (r_roche > 0 && !(fission_product_radius(M_star_msun, lambda, R_A) > r_roche))
        return false;
    return true;
}
...
function fission_position(M_star_msun, lambda, R_A) {
    if (!(R_A > 0))
        return 0;
    if (in_fission_edge(M_star_msun, lambda, R_A))
        return fission_product_radius(M_star_msun, lambda, R_A);
...
function stellar_radius_au(M_star) {
    if (!(COMP_MDOT > 0))
        return 0; // no composition context (legacy path)
    return Math.pow(M_star * FLIFT_MSUN / ((4 / 3) * Math.PI * stellar_mean_density_cgs()), 1 / 3) / FLIFT_AU_CM;
}
function roche_limit_au(M_star) {
