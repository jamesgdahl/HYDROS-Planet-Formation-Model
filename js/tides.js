...
function tide_seats(M_star, spin, f_disc, slots) {
    const pos = cascade_slot_positions(M_star, spin, 10);
    const snow = snow_line(M_star, f_disc);
    const seats = [];
    for (let n = 0; n < pos.length; n++) {
        const m = slot_predicted_mass(pos[n], M_star, spin, f_disc, undefined, undefined);
        if (!(m > 0.01))
            continue;
...
function tide_ledger(seats, M_star, M_pert, q, a_bin) {
