...
### Wavelength doubling (the Suhl parametric subharmonic)

In a **weak magnetic field** the cascade standing wave **period-doubles**: it decays to half its rung frequency, so the slots space out by 2× (α_eff = CASCADE_ALPHA / 2). This is the **Suhl spin-wave instability** — the magnetic-resonance analogue of the Faraday wave, where a driven system responds at *half* the pump frequency once it crosses a drive-vs-damping threshold. (Parametric instability only ever cascades *down* in frequency, never up — which is why an additive *2nd harmonic* / "half-rung" population was always unphysical and was removed; it had been inert in any case.) The 1×→2× onset is a 2-D **Faraday tongue**, not a flat threshold (a flat `B_crit` is impossible: 47 UMa doubles at B=0.19 while HD 20794 stays 1× at *lower* B=0.10). Faraday tongues are **periodic** (a 2× resonance at each integer N/2), and each tongue **widens with cavity size** (Q ∝ N): field-tolerance `B_width(N) = 0.155·(N−7)` opening from the tip at N=7 (N/2=3.5). So a wide cavity tolerates a strong field — 47 UMa (N=8.6) doubles at B=0.19, and **Alpha Cen (N=19.8, N/2≈10) doubles even at B≈1** — while near-tip (HD 20794) or strong-field-modest-cavity systems (Sol, HR 8799) stay fundamental. (Forced to 1×, Alpha Cen invents a 6,240 M⊕ phantom brown dwarf to soak up its budget overestimate; 2× is the parsimonious fit, the residual just exposing that over-budget — orthogonal to the regime.) All inputs are pre-formation (R_A, R_disc, B) — the observed planets never enter the decision (the field exists before the planets, so what forms can't set the regime). Doublers: Beta Pictoris (14.9% → **0.001%**), HD 142, HD 134987, 55 Cnc, ups And, HD 219134, HD 60532, 47 UMa, and Alpha Cen. The **depth** (2× vs 4×) is a genuine **Feigenbaum period-doubling cascade** in N: the bifurcation points crowd toward an accumulation N_∞ with the universal ratio δ = 4.66920…, so the f/4 floor is δ-*derived* (`N₂ = N_∞ − (N_∞ − N₁)/δ ≈ 8.04`), not calibrated — and the highest-N system (Beta Pic) is *not* the deepest precisely because it is past N_∞. HD 142 → 4× → **0.0003%**. See [wavelength-doubling.md](wavelength-doubling.md).
