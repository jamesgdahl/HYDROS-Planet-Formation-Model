...
function equilibrium_pressure(M_star: number, R_disc: number, f_disc: number): number {
...
function wind_flux(M_star: number): number {
...
function gas_inner_edge(M_star: number, R_disc: number, f_disc: number): number {
...
function consumption_front(M_star: number, omega: number, f_disc: number):
    { tedge: (r: number) => number; r0: number } {
  ...
  for (let t = dt; t <= T_MAX; t += dt) {
    ...
    traj.push([t, R_inner]);
    if (R_inner > 200) break;
  }
  const tedge = (r: number): number => {
    if (r <= traj[0][1]) return 0;                         // already inside the front (in the magnetosphere)
...
function stellar_hydrogen_rate(M_star: number, R_disc: number, f_disc: number): number {
...
function disc_centrifugal_radius(M_star: number, spin: number): number {
...
function dynamo_field_strength(M_star: number, spin: number): number {
  const M_E = M_star * M_SUN_TO_EARTH;
  const rock = M_E * COMP_Z * COMP_F_ROCK;
  const H = M_E * (1.0 - COMP_Z);
  const B_REL_SOL = Math.pow(M_SUN_TO_EARTH / M_SUN_EARTH, DYNAMO_SAT_EXP);
...
function wind_lever_arm(M_star: number, spin: number): number {
  const M_E = M_star * M_SUN_TO_EARTH;
...
function wind_alfven_radius_rel(M_star: number, spin: number): number {
  const M_E = M_star * M_SUN_TO_EARTH;
...
function wind_density_slope(M_star: number, spin: number): number {
...
function orbital_period(r: number, M_star: number): number {
...
function gas_consumption_time(M_star: number, f_disc: number): number {
...
function aspect_ratio(r: number): number {
...
function pebble_isolation_mass(r: number, M_star: number): number {
