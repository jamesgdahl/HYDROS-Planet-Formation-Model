...
function apply_hydrogen_conservation(slots: FitSlot[], reservoir: number, M_star: number, spin: number,
  ...
  const is_frag = (s: FitSlot) => !!s.core_fragment;
  ...
  const claims = (s: FitSlot) => !s.external && !s.exterior && !is_frag(s) && (s.predicted - s.core) > 1e-9;
  ...
  const star_frac = Math.min(1, Math.max(0,
    STAR_CONSUME_K * Math.pow(Math.max(COMP_Z, 1e-9) / STAR_Z_SOL, -STAR_METAL_EXP)
  ...
  if (wsum >= planet_budget) {
    if (wsum > 0) for (let i = 0; i < recipients.length; i++) {
      const g = planet_budget * wants[i] / wsum;
      setgas(recipients[i], g); captured += g;
    }
  } else {
