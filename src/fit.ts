...
function slot_aware_fit(planets: Planet[], M_star: number,
  ...
  if (auto_compress && (spin === undefined || spin === null)) {
    ...
    spin = auto_spin_with_anchor_search(
      planets.filter(p => !p.kbo && !p.core), M_star, f_disc).spin;
  ...
  for (const s of slot_data) {
    ...
    if (!gas_eligible) {
      ...
      for (let i = 0; i < max_iter; i++) {
        ...
        if (total > target_mass) lo = t_form; else hi = t_form;
        t_form = (lo + hi) / 2;
      }
  ...
  if (kbo_bodies.length > 0) {
    ...
    for (const p of kbo_bodies) {
      ...
      let vintage: string;
      if (onset) vintage = `<${t_disc_myr.toFixed(1)} Myr (firehose onset)`;
...
function bestFit(planets: Planet[], M_star: number, f_disc_initial: number,
  ...
  const immutable_names = new Set(
    planets.filter(p => p.immutable).map(p => p.name));
  const sel = (f: FitResult): FitSlot[] => (immutable_names.size > 0
...
function apply_sign_modulation(slots: FitSlot[], C: number,
                               ...
                               M_star?: number, f_disc?: number,
                               immune?: Set<string>): void {
...
function budgetFit(planets: Planet[], budget: Budget,
  ...
  try {
    ...
    const Mdot_of = (fd: number) => accretion_rate(M, Z, f_rock, fd, outermost, B, C);
    ...
    const sel = (fit: FitResult): FitSlot[] => fit.slots.filter(s =>
    ...
    const snowA = inverted
      ? Math.max(pile_snow_line(M, fA, alfven_radius(M, SNOW_REF_SPIN), SNOW_REF_SPIN),
