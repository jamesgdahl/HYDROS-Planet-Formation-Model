...
function factory_product(r: number, M_star: number, omega: number, f_disc: number):
  ...
  if (inverted_here) {
    ...
    const INV_PILE_SCALE = (globalThis as any).__INV_PILE || 0.8;    // pile scale in units of R_in (<1 ⇒ declining rock); TRAPPIST-calibrated
    const T_ACC_COEF = (globalThis as any).__T_ACC || 1.0e4;         // accretion-heating temperature coefficient; sets the ice (T_acc<270) transition
    const R_pile = Math.max(INV_PILE_SCALE * R_in, 1e-9);
    const norm = R_pile * (Math.exp(-R_in / R_pile) - Math.exp(-R_outer / R_pile));
    if (!(norm > 0)) return { total: 0, rock: 0, ice: 0 };
    ...
    const R_gas = ((globalThis as any).__INV_WATER || 2.0) * R_A;   // water piled at the cold front (~R_A), TRAPPIST-calibrated
    ...
    if (Sig_solid <= 0) return { total: 0, rock: 0, ice: 0 };
