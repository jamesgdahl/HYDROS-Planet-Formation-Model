...
interface ForensicsSource { slot: number; mass: number; from: string; }

interface ForensicsDating {
  name: string; kept: number; t_form: number; tstar: number;
}

interface ForensicsBareCore {
  name: string; t_form: number; core: number; observed: number;
  err_pct: number;
}

interface ForensicsRow {
  target: string; srcSlot: number; srcMass: number;
  dv: number; vesc: number; ret: number;
  da_net: number; da_obs: number;
  t_form: number; kept: number;
}

interface ForensicsHypothesis {
...
function impact_forensics(all_slots: FitSlot[], planets: Planet[],
                          M_star: number, f_disc: number): Forensics {
  const slots = all_slots.filter(s => !s.exterior && !s.external);

  // gas e-fold for this system (mirrors hydrogen_capture)
  const k = 0.691 * Math.max(1.0, Math.pow(
    (0.01 * m_star_earth(1.0)) / (f_disc * m_star_earth(M_star)), 2.0));

  // --- candidate impactors: dispersed-zone primaries ------------------
  const sources: ForensicsSource[] = [];
  ...
  const zones: ForensicsZone[] = sources.map(src => {
  ...
  // --- gas targets and truncation dating ------------------------------
  interface GasTarget { s: FitSlot; core: number; gas_pred: number; gas_obs: number; }
  const gasTargets: GasTarget[] = [];
  for (const s of slots) {
    if (!s.filled || s.observed <= 0) continue;
  ...
  const neverGassed: ForensicsBareCore[] = [];
  if (tstar !== null) {
    for (const t of gasTargets) {
      if (t.s.t_form > tstar) {
        neverGassed.push({
  ...
  const hypotheses: ForensicsHypothesis[] = [];
  let verdict: ForensicsRow[] | null = null;
  if (tstar !== null && sources.length >= 1) {
    const ts_final: number = tstar;
    ...
    if (struck.length >= 1 && prims.length >= struck.length) {
      const perm = (arr: ForensicsSource[]): ForensicsSource[][] =>
        arr.length <= 1 ? [arr] :
        arr.flatMap((x, i) =>
          perm(arr.slice(0, i).concat(arr.slice(i + 1))).map(rr => [x, ...rr]));
      const launch = (sn: number): number => {
      ...
      for (const ass of perm(prims.slice(0, struck.length))) {
        ...
        if (score < bestScore) { bestScore = score; verdict = rows; }
      }
    }
  }
