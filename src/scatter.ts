...
function decompose_scatter(target: FitSlot, perturber: FitSlot, perturberName: string,
                           outwardTargetName: string, outwardTargetAu: number,
                           inwardAu: number, inwardFate: string): void {
  ...
  const sq = (x: number) => x > 0 ? Math.sqrt(x) : 0;
...
function outer_giants(results: FitSlot[], perturber: FitSlot): FitSlot[] {
...
function inward_perihelion(parent_seat_au: number, perturber_seat_au: number): number {
...
function inner_collision_target(results: FitSlot[], parent_form_au: number, q: number,
                                scattered: Set<FitSlot>): FitSlot | null {
  if (!(q < parent_form_au)) return null;                  // perihelion above the parent ⇒ no inward crossing
  let best: FitSlot | null = null;
  for (const s of results) {
    if (s.external || s.exterior || s.core_component) continue;
    if (scattered.has(s) || s.remnant || s.destroyed) continue;  // fellow scatter products — passed through
    if (s.predicted >= 10) continue;                       // a giant is an eviction, not an impact victim
    const r = (s.form_r && s.form_r > 0) ? s.form_r : s.slot_r;
    if (!(r > 0) || r >= parent_form_au || r < q) continue; // must lie within the orbit [q, parent]
    if (!best || r > ((best.form_r && best.form_r > 0) ? best.form_r : best.slot_r)) best = s;  // OUTERMOST in reach (first crossed)
  }
  return best;
}

function apply_scattering(results: FitSlot[], M_star: number, R_A_now: number,
                          planet_by_slot: Record<number, Planet>): void {
  ...
  for (let i = 0; i < results.length - 1; i++) {
    ...
    if (outer.exterior || inner.exterior) continue;
    if (!outer.filled || inner.filled) continue;   // survivor is the FILLED body; impactor an empty slot
    ...
    if (inner.slot_r <= 1.3 * R_A_now && outer_rocky > 0 && outer.primordial.core < 3.0) {
      ...
      if (v_launch > v_esc_m) {
        ...
        outer.rock = iron; outer.ice = 0; outer.pebble = 0; outer.h_he = 0; outer.core = iron;
        outer.remnant = true;
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (target.filled || target.external) continue;
    if (!target.slot_r || target.slot_r <= 0) continue;
    if (target.predicted < 0.05) continue;
    for (let j = 0; j < results.length; j++) {
      if (i === j) continue;
      const neighbor = results[j];
      if (!neighbor.filled || neighbor.external) continue;
      if (Math.abs(neighbor.slot_n - target.slot_n) > 1) continue;  // adjacent only
      const m_n = Math.max(neighbor.observed, neighbor.predicted);
      const m_t = target.predicted;
      if (m_n <= m_t) continue;  // neighbor must be larger
      ...
      if (sep / R_H_mutual >= LISSAUER_THRESHOLD) continue;
  ...
  interface ScatterMatch {
    ...
    // FORWARD-ONLY: every position is a PREDICTED formation seat (slot_r). The old
    // model added a second "arrival" epoch at the perturber's OBSERVED position to let
    // a migrated giant threaten its new neighbourhood — but a forward prediction cannot
    // read observed migration, so survivors settle relative to the predicted seat alone.
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (!perturber.slot_r || perturber.slot_r <= 0) continue;
      if (perturber.external) continue;
  ...
  const scattered_targets = new Set<FitSlot>();
  for (const i of target_perturbers.keys()) scattered_targets.add(results[i]);
  const scatter_groups = new Map<string, ScatterMatch[]>();
  const multi_perturbed: ScatterMatch[][] = [];
  for (const matches of target_perturbers.values()) {
    if (matches.length >= 2) {
      ...
      if (!scatter_groups.has(key)) scatter_groups.set(key, []);
  ...
  for (const [perturberName, group] of scatter_groups) {
    ...
    const ordered = [...group].sort((a, b) =>
      Math.abs(a.target_r - a.perturber_r) - Math.abs(b.target_r - b.perturber_r));
    const outAssign = new Map<ScatterMatch, { name: string; au: number }>();
    ordered.forEach((g, i) => {
    ...
    const r_in_fallback = (g: ScatterMatch) =>
      boundary_valid ? r_boundary : 0.4 * ((g.target.form_r && g.target.form_r > 0) ? g.target.form_r : g.target.slot_r);
    for (const g of group) {
      ...
      if (g === settled && boundary_valid) {
        ...
        decompose_scatter(target, perturberSlot, perturberName, o.name, o.au,
                          r_boundary, `settled inward survivor (${target.name})`);
      } else {
