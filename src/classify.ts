...
function equilibrium_temperature(r: number, M_star: number): number {
...
function mantle_strip_fraction(r: number, M_star: number): number {
...
function composition_class(core_v: number, ice_v: number, h_he_v: number, total_v: number): string {
  if (total_v >= M_O_DWARF)    return "O-class star";
  if (total_v >= M_B_DWARF)    return "B-class star";
  if (total_v >= M_A_DWARF)    return "A-class star";
  if (total_v >= M_F_DWARF)    return "F-class star";
  if (total_v >= M_G_DWARF)    return "G-class star";
  if (total_v >= M_K_DWARF)    return "K-class star";
  if (total_v >= M_STELLAR_IGNITE) return "M-class star";
  ...
  if (core_v > 0 && ice_v / core_v >= ICE_GIANT_ICE_FRAC) return "icy";
  return "rocky";
}
...
function classify_slot(slot: ClassifySlotInfo, primordial: Composition,
                       r_snow: number, migrated: boolean,
                       _migrants?: string[]): string {
  ...
  const primordial_comp = composition_class(primordial.core || 0,
                                    primordial.ice || 0,
                                    primordial.h_he || 0,
                                    primordial.total || 0);
...
function effective_mass_for_assignment(planet: Planet, M_star: number): number {
  if (is_stripped(planet, M_star)) return (planet.observed || 0) / IRON_FRACTION;
